...
tb <- read.csv("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/limma_GTEx_aging_female.txt", sep="")
...
fulldata = fulldata[which(fulldata$sex == "FEMALE"),]
...
fulldata_aggregated = fulldata %>% group_by(age_group)  %>%
  summarise(mean_pathScore = mean(indegree_increasing))
head(fulldata_aggregated)


ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title="GTEx female: genes increasingly targeted with age ",
                                                               y="gene score", x = "age")
...
ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title="GTEx female: genes decreasingly targeted with age ",
                                                               y="gene score", x = "age")
...
ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title="GTEx female: change in aging_score with age ",
                                                               y="aging score", x = "age")

...
GTEx_disease_below50 = get(load(paste("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/GTEx_Thyroid_diseaseEffect_", disease, "_below50_GSEA.RData", sep = "")))
GTEx_disease_above50 = get(load(paste("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/GTEx_Thyroid_diseaseEffect_", disease, "_above50_GSEA.RData", sep = "")))

tab = data.frame(GTEx_age$NES)
rownames(tab) = GTEx_age$pathway
tab$below50 = GTEx_disease_below50$NES[match(rownames(tab), GTEx_disease_below50$pathway)]
tab$above50 = GTEx_disease_above50$NES[match(rownames(tab), GTEx_disease_above50$pathway)]
colnames(tab) = c("age effect", "disease effect (age<=50)", "disease effect (age>50)")
head(tab)

tab_pval = data.frame(GTEx_age$padj)
rownames(tab_pval) = GTEx_age$pathway
tab_pval$below50 = GTEx_disease_below50$padj[match(rownames(tab_pval), GTEx_disease_below50$pathway)]
tab_pval$above50 = GTEx_disease_above50$padj[match(rownames(tab_pval), GTEx_disease_above50$pathway)]
colnames(tab_pval) = c("age effect", "disease effect (age<=50)", "disease effect (age>50)")
head(tab_pval)
...
pdf(paste("/home/esaha/Aging_thyroid/results/plots/heatmap/GTEx_thyroid_diseaseEffect_", disease, "_byAgeGroup_heatmap.pdf", sep = ""),h=25,w=16)

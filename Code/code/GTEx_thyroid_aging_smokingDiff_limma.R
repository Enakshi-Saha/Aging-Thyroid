...
#### Design Matrix
design = model.matrix(~ age*smoking_status + gender + race + bmi + ischemic_timeH + rna_degrad + batch_effect +
                        smoking_status)
...
contrast.matrix <- makeContrasts(ageSmoker = age + age.smoking_statusYes, levels=design)
colnames(fit$coefficients) = rownames(contrast.matrix)
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)
# tb = topTable(fit2,coef="ageSmoker",number=Inf)
...
save(fgseaRes, file = "/home/esaha/Aging_thyroid/results/limma_GSEA_tables/GTEx_Thyroid_Aging_SexDiff_agingNonsmoker_GSEA.RData")
# save(fgseaRes, file = "/home/esaha/Aging_thyroid/results/limma_GSEA_tables/GTEx_Thyroid_Aging_SexDiff_agingSmoker_GSEA.RData")

write.table(tb, "/home/esaha/Aging_thyroid/results/limma_GSEA_tables/limma_GTEx_aging_nonsmoker.txt")
# write.table(tb, "/home/esaha/Aging_thyroid/results/limma_GSEA_tables/limma_GTEx_aging_smoker.txt")

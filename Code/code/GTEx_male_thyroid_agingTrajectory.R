...
tb <- read.csv("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/limma_GTEx_aging_male.txt", sep="")
...
fulldata = fulldata[which(fulldata$sex == "MALE"),]
...
ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title="GTEx male: genes increasingly targeted with age ",
                                                               y="gene score", x = "age")
...
ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title="GTEx male: genes decreasingly targeted with age ",
                                                               y="gene score", x = "age")
# Summarize data for each age
fulldata_aggregated = fulldata %>% group_by(age_group)  %>%
...
ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title="GTEx male: change in aging_score with age ",
                                                               y="aging score", x = "age")

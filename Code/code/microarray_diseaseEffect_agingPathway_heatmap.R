...
datasetID = "GSE29315"
directory=paste("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/", datasetID, "/sexAgnosticGSEA/", sep = "")
setwd(directory)

# Load GSEA tables 
files_ls = list.files(path=".", pattern=".RData", all.files=TRUE,full.names=TRUE)
...
for (i in 1:length(files_ls)){
  f = files_ls[i]
  disease = unlist(strsplit(f,split="_"))[3]
  fgsea = get(load(f))
...
# Remove pathways with NA everywhere
tab_subset = na.omit(tab_subset)

mycol2 <- colorRampPalette(rev(brewer.pal(11,"RdBu")))(50)
pdf(paste("/home/esaha/Aging_thyroid/results/plots/heatmap/", datasetID, "diseaseEffect_agingPathway_heatmap.pdf", sep = ""),h=25,w=16)

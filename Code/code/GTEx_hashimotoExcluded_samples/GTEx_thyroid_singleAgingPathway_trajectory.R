...
sexDiff_paths = GTEx$pathway[which(GTEx$padj<0.05)]
...
pdf("/home/esaha/Aging_thyroid/results/plots/ageTrajectory/GTEx_thyroid_hashimotoExcluded/KEGG_singleAgingPathway_trajectory.pdf")
for (i in 1:length(sexDiff_paths)){
  pathway_name = sexDiff_paths[i]
  
  pathgenes = unlist(pathways[which(names(pathways) == pathway_name)])
  ...
  pathway_name_lowercase = tolower(gsub("KEGG_", "", pathway_name))

...
# save(fgseaRes, file = "/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/GTEx_hashimotoExcluded_samples/GTEx_Thyroid_aging_GSEA.RData")

# write.table(tb, "/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/GTEx_hashimotoExcluded_samples/limma_GTEx_aging.txt")

#########################
# Significant genes
pos_genes = tb$gene_name[which(tb$P.Value < 0.05 & tb$t > 0)]
neg_genes = tb$gene_name[which(tb$P.Value < 0.05 & tb$t <= 0)]
...
# Significant genes
intersect(oncogene, pos_genes)
intersect(oncogene, neg_genes)

intersect(TSG, pos_genes)
intersect(TSG, neg_genes)

intersect(escape_gene_names, pos_genes)
intersect(escape_gene_names, neg_genes)

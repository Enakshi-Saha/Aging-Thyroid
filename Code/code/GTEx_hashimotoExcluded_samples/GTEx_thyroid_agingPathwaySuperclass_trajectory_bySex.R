...
pdf("/home/esaha/Aging_thyroid/results/plots/ageTrajectory/GTEx_thyroid_hashimotoExcluded/KEGG_agingPathwaySuperclass_trajectory.pdf")
...
pdf("/home/esaha/Aging_thyroid/results/plots/ageTrajectory/GTEx_thyroid_hashimotoExcluded/KEGG_agingPathway_8Superclass_trajectory_v2.pdf")
for (i in 1:length(pathway_superclass)){
  ...
  if (length(pathList) != 0){
    ...
    p = ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore, color = sex)) +
      geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title=paste("GTEx: pathways related to",pathway_name_lowercase, "with age"),
                                                                   y="gene score", x = "age")
    print(p)
  }
}

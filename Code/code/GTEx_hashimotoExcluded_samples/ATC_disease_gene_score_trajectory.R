...
diseaseEffect_ATC1 = data.frame(read.csv("~/Aging_thyroid/results/limma_GSEA_xcell_tables/GSE33630/limma_GSE33630_diseaseEffect_anaplastic thyroid carcinoma (ATC).txt", sep=""))
diseaseEffect_ATC2 = data.frame(read.csv("~/Aging_thyroid/results/limma_GSEA_xcell_tables/GSE27155/limma_GSE27155_diseaseEffect_Anaplastic Thyroid Carcinoma.txt", sep=""))

disease_gene_increasing1 = rownames(diseaseEffect_ATC1)[which(diseaseEffect_ATC1$P.Val < 0.05 & diseaseEffect_ATC1$t > 0)]
disease_gene_decreasing1 = rownames(diseaseEffect_ATC1)[which(diseaseEffect_ATC1$P.Val < 0.05 & diseaseEffect_ATC1$t < 0)]

disease_gene_increasing2 = rownames(diseaseEffect_ATC2)[which(diseaseEffect_ATC2$P.Val < 0.05 & diseaseEffect_ATC2$t > 0)]
disease_gene_decreasing2 = rownames(diseaseEffect_ATC2)[which(diseaseEffect_ATC2$P.Val < 0.05 & diseaseEffect_ATC2$t < 0)]
...
ggplot(fulldata_aggregated, aes(x = age_group , y = increasing_score_1, color = sex)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title=paste("GTEx: Age-trajectory of genes upregulated in ATC"),
                                                               y="gene score", x = "age")
ggplot(fulldata_aggregated, aes(x = age_group , y = decreasing_score_1, color = sex)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title=paste("GTEx: Age-trajectory of genes downregulated in ATC"),
                                                               y="gene score", x = "age")
...
ggplot(fulldata, aes(x = age_group , y = increasing_score, fill = sex)) +
  ...
  geom_half_boxplot(side = "left") + geom_half_violin(side = "right") + labs(title="Genes upregulated in ATC",
...
ggplot(fulldata, aes(x = age_group , y = decreasing_score, fill = sex)) +
  scale_fill_manual(values=c("purple1", "lightgreen")) +
  geom_half_boxplot(side = "left") + geom_half_violin(side = "right") + labs(title="Genes downregulated in ATC",
                                                                             y="gene score", x = "age group") +

...
# targeting of oncogenes: aging trajectory
cosmic_gene_name = "TNFAIP3"
targeting_score = as.numeric(bonobo[which(gene_name == cosmic_gene_name),])
...
fulldata$targeting_score = targeting_score
...
fulldata_aggregated = fulldata %>% group_by(age_group, sex)  %>%
  summarise(mean_targeting = mean(targeting_score))
...
ggplot(fulldata_aggregated, aes(x = age_group , y = mean_targeting, color = sex)) +
  geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title=paste("GTEx:", cosmic_gene_name, "targeting by TFs with age", sep = " "),
                                                               y="gene score", x = "age")

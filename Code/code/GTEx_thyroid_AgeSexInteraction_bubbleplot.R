...
fgseaRes = get(load("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/GTEx_Thyroid_Aging_SexDiff_agingSexInteraction_GSEA.RData"))

# Choose colors as ggplot2's default colors for 2 categories
library(scales)
library(colorspace)
defaultcols = hue_pal()(2)
# darken 30%
defaultcols <- darken(defaultcols, 0.2)
...
dencol_neg <- defaultcols[2] # bubble plot color for negative ES
dencol_pos <- defaultcols[1] # bubble plot color for positive ES
signnamelength <- 4 # set to remove prefix from signature names (4 for "GOBP", 4 for "KEGG", 8 for "REACTOME")

...
pdf("/home/esaha/thyroid_revision_results/revision_plots/KEGG_agingPathway_8Superclass_trajectory_rawAge.pdf")
for (i in 1:length(pathway_superclass)){
  ...
  if (length(pathList) != 0){
    ...
    # Remove aggregation — use individual-level data with raw age
    fulldata$sex = factor(fulldata$sex, levels = c("FEMALE", "MALE"))
    
    p = ggplot(fulldata, aes(x = age, y = indegree, color = sex)) +
      geom_point(size = 0.5, alpha = 0.3) +   # alpha helps with overplotting
      geom_smooth(method = "loess", method.args = list(span = 0.75)) +
      labs(
        title = paste(pathway_name_lowercase),
        y = "score",
        x = "age"
      ) +
      theme(

...
#### Design Matrix
# gender = relevel(gender, ref = levels(gender)[2]) # run this line before finding effect for the other gender
...
for (d in diseases){
  ...
  # save limma table
  write.table(tb, file = paste("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/", datasetID, "/limma_", datasetID, "_", levels(gender)[1], "_diseaseEffect_", d, ".txt",sep = ""))
  ...
  # save GSEa table
  save(fgseaRes, file = paste("/home/esaha/Aging_thyroid/results/limma_GSEA_xcell_tables/", datasetID, "/", datasetID, "_", levels(gender)[1], "_diseaseEffect_", d, "_GSEA.RData",sep = ""))
}

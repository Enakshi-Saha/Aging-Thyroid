...
# Load KEGG pathways
pathways <- gmtPathways("/home/esaha/thyroid_revision_data/restored_data/c2.cp.kegg.v2022.1.Hs.symbols.gmt")

# Load KEGG pathway hierarchy
pathway_hierarchy = get(load("/home/esaha/thyroid_revision_data/KEGG_pathway_hierarchy_byFunction.RData"))

# Get GTEx aging pathways
# Load GSEA tables 
GTEx_gsea = get(load("/home/esaha/thyroid_revision_results/limma_GSEA_xcell_tables/GTEx_hashimotoExcluded_samples/GTEx_Thyroid_aging_GSEA.RData"))
...
##############################
bonobo = data.frame(fread("/home/esaha/thyroid_revision_data/restored_data/BonoboPanda_GTEx_thyroid.txt"))
genes = bonobo$V1
bonobo = bonobo[,-1]
...
# Get phenotypic data
phenotypes <- read.csv("/home/esaha/thyroid_revision_data/restored_data/thyroid_phenotypes.txt", sep="")
...
pdf("/home/esaha/thyroid_revision_results/revision_plots/KEGG_agingPathwaySuperclass_trajectory.pdf")
for (i in 1:length(pathway_hierarchy)){
  ...
  if (length(pathList) != 0){
    ...
    p = ggplot(fulldata_aggregated, aes(x = age_group , y = mean_pathScore, color = sex)) +
      geom_point(size = 0.5) + geom_smooth(method = "auto") + labs(title=paste("GTEx: targeting of pathways associated to\n",pathway_name_lowercase, "with age"),
                                                                   y="gene score", x = "age")
    print(p)
  }
}
...
pathgroups = lapply(pathway_hierarchy, function(x){intersect(sig_pathways,x)})
pathgroups[which(lapply(pathgroups, length) > 0)]
...
# Leave-one-bin-out sensitivity analysis
bins = unique(fulldata$age_group)

pdf("/home/esaha/thyroid_revision_results/revision_plots/KEGG_agingPathway_8Superclass_leaveOneBinOut.pdf")
for (i in 1:length(pathway_superclass)){
  ...
  if (length(pathList) != 0){
    ...
    pathway_name_lowercase = stringr::str_to_title(pathway_class)
    
    # Full data curve
    fulldata_aggregated = fulldata %>% group_by(age_group, sex) %>%
      summarise(mean_pathScore = mean(indegree), .groups = "drop")
    fulldata_aggregated$sex = factor(fulldata_aggregated$sex, levels = c("FEMALE", "MALE"))
    
    # Leave-one-bin-out curves
    lobo_curves = lapply(bins, function(b){
      fulldata %>%
        filter(age_group != b) %>%
        group_by(age_group, sex) %>%
        summarise(mean_pathScore = mean(indegree), .groups = "drop") %>%
        mutate(left_out_bin = b,
               sex = factor(sex, levels = c("FEMALE", "MALE")))
    })
    lobo_df = bind_rows(lobo_curves)
    
    p = ggplot() +
      # Leave-one-bin-out curves in the background
      geom_smooth(data = lobo_df,
                  aes(x = age_group, y = mean_pathScore,
                      color = sex, group = interaction(sex, left_out_bin)),
                  method = "loess", se = FALSE, alpha = 0.2, linewidth = 0.4) +
      # Main curve on top
      geom_smooth(data = fulldata_aggregated,
      ...
      labs(title = paste(pathway_name_lowercase),
           y = "score", x = "age") +
